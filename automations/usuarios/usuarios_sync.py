...
def _extract_entity_field(obj: dict, field: str) -> tuple[str | None, str | None]:
    """Extrai (id, name) de um campo que pode ser dict ou int."""
    raw = obj.get(field)
    if raw is None:
        return None, None
    if isinstance(raw, dict):
        ...
        return (str(fid), fname) if fid else (None, None)
    try:
        v = int(raw)
        return (str(v), "") if v else (None, None)
    except (ValueError, TypeError):
        return None, None

def _user_cc(u: dict) -> tuple[str | None, str | None]:
    """CC do usuário = entidade padrão (default_entity)."""
    return _extract_entity_field(u, "default_entity")

def _comp_cc(comp: dict) -> tuple[str | None, str | None]:
    """CC do computador = campo entity."""
    return _extract_entity_field(comp, "entity")
